...
Same content`);
  ...
  assert.ok(claudeContent.includes('@../../../.sync-skills/skills/order-skill/SKILL.md'));
  assert.ok(codexContent.includes('@../../../.sync-skills/skills/order-skill/SKILL.md'));

  // Common skill should exist
  const commonContent = await fs.readFile(join(testDir, '.sync-skills/skills/order-skill/SKILL.md'), 'utf8');
