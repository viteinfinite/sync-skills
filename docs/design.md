...
`syntax-skills` is an npm package (runnable via npx) that synchronizes agent skill definitions between `.claude/` and `.codex/` directories. It extracts common content to `.sync-skills/` and uses `@`-based static linking for shared definitions.

**Key features:**
- Bidirectional sync between `.claude/` and `.codex/` skills
- Automatic refactoring: extracts shared content to `.sync-skills/`
...
```
your-project/
├── .sync-skills/               # Shared skill content (no frontmatter)
│   └── skills/
│       └── pr-review/
...
### Source File (`.sync-skills/skills/*/SKILL.md`)
...
@.sync-skills/skills/pr-review/SKILL.md
```
...
.sync-skills/skills/*.md  →  Common content files
```
...
If a skill doesn't have an `@` reference, extract its body to `.sync-skills/`:
...
```markdown
# .sync-skills/skills/pr-review/SKILL.md (created)
...
function refactorSkill(sourcePath, targetAgent) {
  ...
  const commonPath = `.sync-skills/skills/${skillName}/SKILL.md`;

  // 1. Write body to .sync-skills
...
- Tracks when content was extracted to `.sync-skills/`
...
metadata:
  sync:
    managed-by: sync-skills
    refactored: 2025-01-11T10:30:00Z
    content-hash: abc123...    # Hash of .sync-skills content
