 ...
 */
async function installSyncSkillsSkill(baseDir) {
...
export async function run(options = {}) {
    let { baseDir = process.cwd(), failOnConflict = false, homeMode = false, reconfigure = false, listMode = false, installSelfSkill = false } = options;
    // Handle --home flag
    if (homeMode) {
        if (!process.env.HOME) {
        ...
        try {
            ...
            if (parsed?.hasAtReference && !description) {
                ...
                try {
                    ...
                    description = refParsed?.data?.description || '';
                }
                catch {
