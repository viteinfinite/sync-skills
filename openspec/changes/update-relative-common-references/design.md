## Context
Platform skills currently embed `@.sync-skills/skills/<skill>/SKILL.md`, which assumes a root-relative resolution. We need references that are correct relative to each platform skill file regardless of nesting depth and platform directory layout.
...
- Goals:
  ...
  - Treat old `@.sync-skills/...` references as out-of-sync (no backward compatibility).
- Non-Goals:
...
- Keep `@.sync-skills/...` and add lookup logic in consumers.
