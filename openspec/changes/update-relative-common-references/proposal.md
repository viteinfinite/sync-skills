...
## Why
Current @ references use `.sync-skills/...` paths that assume a fixed root. We need nesting-aware references that work reliably across all platforms and locations.

## What Changes
- **BREAKING**: New @ references point to `.sync-skills` via a relative path from each platform skill file (e.g., `@../../../.sync-skills/skills/<skill>/SKILL.md`).
