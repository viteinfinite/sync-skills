...
When a user has `.sync-skills` configured with skills for certain assistants (e.g., claude and gemini), but those assistant directories don't exist yet, the tool currently skips creating them. This is an awkward gap in the workflow:

- Scenario: A team shares `.sync-skills` skills via version control
...
- **ADDED**: Integration test coverage for the scenario where only `.sync-skills` exists
