# Change: Ignore symlinked platform skills pointing to .agents

## Why
Teams sometimes place skills in assistant folders as symlinks to `.agents/skills`, which should not be managed by sync-skills. Today those symlinked skills are treated as real platform skills, leading to incorrect sync/refactor behavior and misleading `--list` output.

## What Changes
- Detect skill directories that are symlinks to `.agents/skills/<skill>` and treat them as unmanaged.
